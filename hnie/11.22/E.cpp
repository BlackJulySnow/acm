...
int main() {
    ...
    int tot = 1;
    vis[0][0] = 1;
    cnt[1].x = 0,cnt[1].y = 0,cnt[1].last = 0;
    q.push(1);
    while (!q.empty()){
        int now = q.front();
        q.pop();
        ...
        if(x == 49 && y == 29){
            while (now != 1){
                int l = cnt[now].last;
//                printf("%d %d %d %d\n",cnt[now].x,cnt[now].y,cnt[l].x,cnt[l].y);
                for(int i = 0;i < 4;i++){
                    if(cnt[l].x + dx[i] == cnt[now].x && cnt[l].y + dy[i] == cnt[now].y){
                        ans[++ansTot] = d[i];
//                        cout << d[i] << endl;
                    }
        ...
        for(int i = 0;i < 4;i++){
            int tx = x + dx[i];
            int ty = y + dy[i];
            if(0 <= tx && tx < 50 && 0 <= ty && ty < 30 && G[tx][ty] == 0 && !vis[tx][ty]){
                q.push(++tot);
                cnt[tot].x = tx;
                cnt[tot].y = ty;
                cnt[tot].last = now;
